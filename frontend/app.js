 ...
 * Real-time spatiotemporal diagnostic curve renderer, Render backend connector, and operator loop.
 */

// Dynamic Backend URL resolution (Vercel -> Render)
const BACKEND_URL = window.BACKEND_API_URL || (
  window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1'
    ? 'http://localhost:8000'
    : 'https://sentinelaix-api.onrender.com'
);
...
// Operator Decision Feedback Loop
window.operatorFeedback = function(feedbackType) {
  ...
  // Send to backend API (Render or local)
